...
/**
 * Searches for nearby restaurants using Foursquare Places API.
...
/**
 * Constructs the category icon URL from Foursquare's prefix/suffix format.
 */
...
const CUISINE_GRADIENTS: Record<string, string> = {
  ...
  Mediterranean: "linear-gradient(135deg, #00897b 0%, #00695c 50%, #004d40 100%)",
...
/**
 * Transforms Foursquare API place data into the internal Restaurant format.
 */
function buildRestaurant(place: FoursquarePlace): Restaurant {
  const primaryCategory = place.categories?.[0];
  const cuisineName = primaryCategory?.short_name || primaryCategory?.name || "Restaurant";
...
/**
 * Mock data generator for development and fallback scenarios.
 */
